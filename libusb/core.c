...
ssize_t API_EXPORTED libusb_get_device_list(libusb_context *ctx,
	libusb_device ***list)
{
	fprintf(stderr, "\nPICODEBUG: libusb_get_device_list(ctx, &list)\n");
...
void API_EXPORTED libusb_free_device_list(libusb_device **list,
	int unref_devices)
{
	fprintf(stderr, "\nPICODEBUG:  libusb_free_device_list(list, unref_devices=%d)\n", unref_devices);
	if (!list)
		return;
...
{
	fprintf(stderr, "\nPICODEBUG: libusb_close(dev_handle)\n");
...
int API_EXPORTED libusb_release_interface(libusb_device_handle *dev,
	int interface_number)
{
	fprintf(stderr, "\nPICODEBUG: libusb_release_interface(device_handle, interface_number=%d)\n", interface_number);
	int r;
...
int API_EXPORTED libusb_clear_halt(libusb_device_handle *dev,
	unsigned char endpoint)
{
	fprintf(stderr, "\nPICODEBUG: libusb_clear_halt(handle=(...),endpoint=%u)\n", endpoint);
